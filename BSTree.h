...
class BSTree
{
	private:
		...
		{
			if(n == nullptr) throw runtime_error("No existe ese arbol");
			...
			else
			{
				...
				else n = (n.left != nullptr)? n.left: n.der;
			}
			return n;
		}
		...
		{
			if(n = nullptr) throw runtime_error("No existe ese arbol");
			else if(n.der != nullptr) return max(n.right);
			else return n.elem;
		}
